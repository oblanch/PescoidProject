...
def pescoid_plot():
    ...
    boundary_velocity_data = np.load(os.path.join(input_dir, 'boundary_velocity_data.npy'))  # Load boundary velocity data
    ...
    assert len(density_data) == len(velocity_data) == len(time_data), \
    "Mismatched lengths of density, velocity, or time data arrays"

    # Ensure length consistency
    assert len(boundary_positions) == len(boundary_time_data) == len(boundary_velocity_data), \
    ...
    density_line, = ax_density.plot(x_coords, density_data[0], color='blue', label='Density')
    mesoderm_line, = ax_density.plot(x_coords, mesoderm_data[0], color='purple', label='Mesoderm')
    velocity_line, = ax_velocity.plot(x_coords, velocity_data[0], color='red', label='Velocity')
    stress_line, = ax_density.plot(x_coords, stress_data[0], color='green', label='stress')
    ...
    velocity_time_text = ax_velocity.text(0.02, 0.95, '', transform=ax_velocity.transAxes)
    ...
    fig_density, update_density_plot, frames=len(density_data), interval=100, blit=True
    )

    ani_velocity = animation.FuncAnimation(
    fig_velocity, update_velocity_plot, frames=len(velocity_data), interval=100, blit=True
    )

    # Save animations
    ani_density.save('/mnt/c/Users/oblanch/FEniCS_tutorial/Dimensional Plots/density_evolution.mp4', writer='ffmpeg')
    ani_velocity.save('/mnt/c/Users/oblanch/FEniCS_tutorial/Dimensional Plots/velocity_evolution.mp4', writer='ffmpeg')
    ...
    plt.grid()
    plt.savefig('/mnt/c/Users/oblanch/FEniCS_tutorial/Dimensional Plots/leading_edge_radius.png')
    plt.show()
    plt.close()
#length_scale, Delta, Da, tau_m, Gamma, Activity
    ...
    plt.title(f'Boundary Velocity over Time (L_0 = {param_list[0]}, delta = {param_list[1]}, F = {param_list[2]}, T_m = {param_list[3]}, Gamma = {param_list[4]}, A = {param_list[5]})')
    plt.grid()
    plt.savefig('/mnt/c/Users/oblanch/FEniCS_tutorial/Dimensional Plots/boundary_velocity.png')
    #plt.show()
    plt.close()
